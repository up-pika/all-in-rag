...
import numpy as np
from pymilvus import WeightedRanker  # 添加 WeightedRanker
...
# 打印加权融合结果
print(
    f"使用加权线性融合，稀疏权重={sparse_weight:.1f}, 密集权重={dense_weight:.1f} (语义优先)："
)
...
print(
    f"使用加权线性融合，稀疏权重={sparse_weight_keyword:.1f}, 密集权重={dense_weight_keyword:.1f} (关键词优先)："
)
...
for i in range(max_len):
    ...
    weighted_title = (
        get_titles(results_weighted)[i] if i < len(results_weighted) else "-"
    )
    keyword_title = get_titles(results_keyword)[i] if i < len(results_keyword) else "-"
    print(
        f"{i+1:2d}    |  {rrf_title:12s}  |  {weighted_title:12s}  |  {keyword_title:12s}"
    )
