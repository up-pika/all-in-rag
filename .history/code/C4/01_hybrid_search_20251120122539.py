...
# 查询满足过滤条件的文档数量
filter_count = collection.query(
    expr=search_filter,
    output_fields=["title"],
    limit=100
)
print(f"满足过滤条件的文档总数: {len(filter_count)} 条")
if len(filter_count) < top_k:
    print(f"⚠️  警告：文档总数({len(filter_count)})少于请求的Top-K({top_k})，实际返回数量会受限！")
...
print(f"密集向量检索返回 {len(dense_results)} 条结果（请求Top-{top_k}）：")
...
print(f"稀疏向量检索返回 {len(sparse_results)} 条结果（请求Top-{top_k}）：")
...
# 创建搜索请求（注意：AnnSearchRequest 中不支持 expr，需要在 hybrid_search 中添加）
...
results = collection.hybrid_search(
    ...
    rerank=rerank,
    limit=top_k,
    expr=search_filter,  # ← 添加过滤器
    output_fields=["title", "path", "description", "category", "location", "environment"]
)[0]

# 打印最终结果
print(f"混合检索（RRF融合）返回 {len(results)} 条结果（请求Top-{top_k}）：")
