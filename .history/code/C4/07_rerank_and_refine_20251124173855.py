...
class DeduplicationCompressor(BaseDocumentCompressor):
    """去重压缩器 - 移除重复或高度相似的内容"""
    ...
    def __init__(self, similarity_threshold: float = 0.8, **kwargs):
        ...
        Args:
            similarity_threshold: 相似度阈值，超过此阈值的文档将被视为重复（0-1之间）
        """
        super().__init__(**kwargs)
    ...
    def _is_duplicate(self, new_text: str, existing_texts: list) -> bool:
        """检查新文本是否与已有文本重复"""
    ...
    def compress_documents(
        ...
        for doc in documents:
            ...
            # 检查是否与已有文档重复
            if not self._is_duplicate(content, seen_texts):
                unique_docs.append(doc)
                seen_texts.append(content)
        
        print(f"去重: {len(documents)} -> {len(unique_docs)} 个文档")
        return unique_docs
...
# 4.1 设置去重压缩器
deduplicator = DeduplicationCompressor(similarity_threshold=0.75)

# 5. 使用DocumentCompressorPipeline组装压缩管道
# 流程: ColBERT重排 -> LLM压缩 -> 去重
pipeline_compressor = DocumentCompressorPipeline(
    transformers=[reranker, compressor, deduplicator]
)
