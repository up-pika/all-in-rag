...
示例代码的运行结果如下：
```python
content='根据上下文，文中举了以下例子：\n\n1.  **自然界中的羚羊**：羚羊出生后通过试错学 习站立和奔跑，以适应环境。\n2.  **股票交易**：通过不断买卖股票并根据市场反馈来学习如何使奖励最大化。\n3.  **玩雅达利游戏或其他电脑游戏**：通过不断试错来学习如何通关。\n4.  **雅达利游戏 Breakout（打砖块）**：作为一个具体的游戏实例，用于说明强化学习的特点，如观测数 据不是独立同分布的以及存在延迟奖励。\n5.  **雅达利游戏 Pong**：作为一个经典例子，说明了 强化学习智能体与正常玩家的区别，以及如何通过策略网络和预演过程进行训练。\n6.  **AlphaGo**：由DeepMind开发的强化学习算法，能够打败人类顶尖棋手，说明了强化学习有超越人类表现的潜 力。' 
additional_kwargs={'refusal': None} 
response_metadata={'token_usage': {'completion_tokens': 189, 'prompt_tokens': 3569, 'total_tokens': 3758, 'completion_tokens_details': None, 'prompt_tokens_details': {'audio_tokens': None, 'cached_tokens': 3520}, 'prompt_cache_hit_tokens': 3520, 'prompt_cache_miss_tokens': 49}, 'model_name': 'deepseek-chat', 'system_fingerprint': 'fp_ffc7281d48_prod0820_fp8_kvcache', 'id': '2303ee3e-e78b-477c-b41a-e12de039dbf9', 'service_tier': None, 'finish_reason': 'stop', 'logprobs': None} id='run--6bae1ac6-19b4-4d81-a704-94b5765c17a8-0' 
usage_metadata={'input_tokens': 3569, 'output_tokens': 189, 'total_tokens': 3758, 'input_token_details': {'cache_read': 3520}, 'output_token_details': {}}
```
优化输出，精简输出具体结果，过滤掉其他结果无关参数后：
![精简结果输出](./images/1_1.png)

修改文本分块实验，分别对比了这几组参数下的生成结果：
  * chunk_size=400, chunk_over=80
  * chunk_size=8000, chunk_over=200
  * 默认参数chunk_size=4000, chunk_over=200
对比结果如下：
![分块实验结果](./images/1_2.png)
可见，缩小chunk_size和chunk_over时，输出的文本不够完整，而增大chunk_size和chunk_over时，输出的文本更加注重细节，结果相对更完整，因此，在保证输出完整性的前提下，需要找到合适的chunk_size和chunk_over值，以避免信息丢失。
