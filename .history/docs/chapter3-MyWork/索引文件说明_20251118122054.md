...
```json
{
  "docstore/metadata": {
    ...
    // ... 其他文档
    // 节点示例（Node，注意包含ref_doc_id字段，指向其所属Document）：
    "dab4a15e-9175-4f2e-803c-1e2f303e30e1": {
      "doc_hash": "bf8d2f004fc7155822678e4e01cdcbc848dc8999d91493b69d94dd23127d96f3",
...
**使用场景**：当使用多模态模型（如 Visual BGE）处理图像时，图像嵌入会存储在这里。
...
**使用场景**：当使用 `KnowledgeGraphIndex` 或 `ComposableGraph` 时，实体和关系会存储在这里。
